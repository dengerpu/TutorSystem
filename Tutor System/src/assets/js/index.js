...
function time() {
  ...
  showTime.innerText = `当前时间：${y}年${m}月${d}日-${hh}时${mm}分${ss}秒`
}

//立即执行函数 防止污染变量
//左侧柱状图    基于Echarts官网的-->坐标轴刻度与标签对齐 
(function(){
  var myChart = echarts.init(document.querySelector('.bar .chart'));

  window.addEventListener("resize", function() {
    myChart.resize();       //自适应大小   echarts提供  
  });
    ...
    xAxis: [
        {
            ...
            //x轴的刻度标签(文字)颜色和大小     注意区分：刻度与刻度标签  刻度是-
            axisLabel: {
        ...
        series: [
            {
                ...
                itemStyle: {
                    normal: {
                        //柱子设为圆角
                        barBorderRadius: 20,    
                        //设置每根柱子颜色  params参数是每根柱子的对象 dataIndex是柱子的引号
                ...
                label: {
                    ...
                    //定位在柱子内部
                    position: "inside",
                    // 文字的显示格式  a是系列名(name) b是数据名(yAxis的data) c是数据值(series的data)
                    formatter: "{c}%"
                },
            },
            {
...
(function(){
    ...
    var eventA = document.querySelector('.line a:nth-child(1)');
    var eventB = document.querySelector('.line a:nth-child(2)');
